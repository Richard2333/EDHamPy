...
@njit
def Vc_mat_part(indb_subset,basis,invbasis,K0,K1,Coulomb_mat,nconf_shift):
    klist = []
...
def lanczos_partial_reortho_CauchyCV_mpi(indb_subset,basis,invbasis,num,numiteration=200,cauchy_tol=1e-6,ortho_tol=1e-6):
    ...
    subset_size2,num_more2=divmod(len(indb_subset),2)
    for i in range(2):
        if i < num_more2:
            indb_subset2 = indb_subset[i*(subset_size2+1):(i+1)*(subset_size2+1)]
            row,col,data = Vc_mat_part(indb_subset2,basis,invbasis,K0,K1,Coulomb_mat,i*(subset_size2+1))
            Ham_mat_part += csr_matrix((data,(row,col)),shape=(length_subset,basis_size),dtype='complex')
        else:
            indb_subset2 = indb_subset[i*subset_size2+num_more2:(i+1)*subset_size2+num_more2]
            row,col,data = Vc_mat_part(indb_subset2,basis,invbasis,K0,K1,Coulomb_mat,i*subset_size2+num_more2)
            Ham_mat_part += csr_matrix((data,(row,col)),shape=(length_subset,basis_size),dtype='complex')

    # row,col,data = Vc_mat_part(indb_subset,basis,invbasis,K0,K1,Coulomb_mat,0)
    # Ham_mat_part += csr_matrix((data,(row,col)),shape=(length_subset,basis_size),dtype='complex')

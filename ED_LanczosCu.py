...
import cupyx.scipy.sparse.linalg as cupy_lin

from cupy.linalg import eigh as ceigh
from cupy.linalg import norm as cnorm
from cupy.linalg import eigvalsh as ceigvalsh
...
if algo == 0:
    ...
    algostr = 'Full Reorthogonalization'
elif algo == 6:
    algostr = 'subcomm mpi+CUDA'
elif algo == 2:
        ...
        for ik in range(klist.shape[0]):
            ...
            if occupation(conf,k3) & occupation(conf,k4):
                ...
                if (1-occupation(newconf,k1)) & (1-occupation(newconf,k2)):
                    ...
                    # rescoef = Vq*sign # <newconf|V|conf>
                    if absolute(rescoef)>1e-14:
                        # indc_new = np.where(basis==newconf)[0][0]
...
def lanczos_mpi_cupy(indb_subset,basis,invbasis,num,numiteration):
    # gpu acceleration by yifei guan
    time0 = time.time()
    # Generate matrix element
    if rank==0:
        print('Start to compute Hamiltonian matrix elements...')
    ...
    Ham_mat_part  = cu_sp.csr_matrix((length_subset,basis_size),dtype='complex')
    row,col,data = Vc_mat_part(indb_subset,basis,invbasis,K0,K1,Coulomb_mat)
    row = cp.array(row)
    col = cp.array(col)
    data = cp.array(data,dtype=cp.complex128)
    Ham_mat_part += cu_sp.csr_matrix((data,(row,col)),shape=(length_subset,basis_size),dtype='complex')
    row,col,data = Ek_mat_part(indb_subset,basis,Ek)
    row = cp.array(row)
    col = cp.array(col)
    data = cp.array(data,dtype=cp.complex128)
    ...
    ########################### MPI ###########################
    if rank == 0:
        u2 = (2.0*cp.random.random_sample(basis_size)+1.0) + 1.j * (2.0*cp.random.random_sample(basis_size)+1.0)
        u2 = u2/cnorm(u2)
    else:
        u2 = None
    ...
    for iter in range(numiteration):
        ...
        u2_measure_part = cp.vdot(u2_part,u2t_part)
        ########################### MPI ###########################
        u2t_allpart = comm.allgather(u2t_part)
        u2t = cp.concatenate(u2t_allpart) # H|u2>
    ...
    tridiag = cp.diag(a)
    tridiag+= cp.diag(b[:-1],k=-1)
    # ~ print( tridiag.shape )
    
    energy = eigvalsh(tridiag.get() )
    
    # ~ energy = ceigvalsh(tridiag).get()
    
    return energy[:num]


def lanczos_reortho_mpi_cupy(indb_subset,basis,invbasis,num,numiteration,ortho_tol=1e-6):
    time0 = time.time()
    # Generate matrix element
    # ~ current_gpu = cp.cuda.Device()
    # ~ print(f"Current GPU: {current_gpu.id}")
    ...
    if rank == 0:
        ...
        print(f'Start to generate iteratively Lanczos matrix...') 
    # Lanczos
    a = cp.zeros(numiteration,dtype=cp.complex128)
    b = cp.zeros(numiteration,dtype=cp.complex128)
    Q = cp.zeros( (numiteration,basis_size) ,dtype=cp.complex128)
    ########################### MPI ###########################
    if rank == 0:
        u2 = (2.0*cp.random.random_sample(basis_size)+1.0) + 1.j * (2.0*cp.random.random_sample(basis_size)+1.0)
        u2 = u2/cnorm(u2)
...
def lanczos_reortho_mpi_cupysub(indb_subset,basis,invbasis,num,numiteration,ortho_tol=1e-6):
    time0 = time.time()
    ...
    if rank==0:
        print('Start to compute Hamiltonian matrix elements...')
    length_subset = len(indb_subset)
    indleader = subcomm.allgather(indb_subset)
    # ~ if rank==0:
        # ~ print(indleader)
    sl = [len(s) for s in indleader]
    # ~ if color == 2:
        # ~ print(sl)
    indleader = np.concatenate(indleader)
    indlen = len(indleader)
    # ~ print(indlen,sl,rank,subrank,sum(sl[:subrank]) )
    
    #不行，vcmatpart 不支持subcomm 方法！
    row,col,data = Vc_mat_part(indb_subset,basis,invbasis,K0,K1,Coulomb_mat)
    row = cp.array(row)+int(sum(sl[:subrank]))
    col = cp.array(col)
    data = cp.array(data,dtype=cp.complex128)
    
    row = subcomm.gather(row, root=0)
    col = subcomm.gather(col, root=0)
    data = subcomm.gather(data, root=0)
    if subrank ==0:
        Ham_mat_part = cu_sp.csr_matrix((indlen,basis_size),dtype='complex')
        row = cp.concatenate(row)
        col = cp.concatenate(col)
        data = cp.concatenate(data)
        Ham_mat_part += cu_sp.csr_matrix((data,(row,col)),shape=(indlen,basis_size),dtype='complex')
    
    row,col,data = Ek_mat_part(indb_subset,basis,Ek)
    row = cp.array(row)+int(sum(sl[:subrank]))
    col = cp.array(col)
    data = cp.array(data,dtype=cp.complex128)
    # ~ print(rank,subrank,row.min(),row.max(),sl[subrank],indlen,int(sum(sl[:subrank])))
    row = subcomm.gather(row, root=0)
    col = subcomm.gather(col, root=0)
    data = subcomm.gather(data, root=0)
    if subrank ==0:
    ...
    # ~ gathered_data = subcomm.gather(data, root=0)
    # ~ if subrank == 0:
        # ~ print(f"Subcomm root (original rank {rank}) gathered data: {gathered_data}")
    # ~ else:
    # ~ # 不参与的进程什么都不做
    # ~ pass
    ...
    color2 = 0 if subrank == 0 else MPI.UNDEFINED
    leader_comm = comm.Split(color=color2, key=rank)
    
    # ~ energy = 
    if leader_comm != MPI.COMM_NULL:
        leader_rank = leader_comm.Get_rank()
        # ~ print(leader_rank,rank,subrank,gpu_id)
        ...
        if leader_rank == 0:
            ...
            u2 = None
        u2 = leader_comm.bcast(u2, root=0)
        ###########################################################
        for ia in range(numiteration):
            time10 = time.time()
            # ~ if rank == 0:
                # ~ if (ia+1)%10 == 0 or ia==0:
                    # ~ print(f'{ia+1}/{numiteration}', end='\t')
            u2t_part = Ham_mat_part.dot(u2)
            u2_part = u2[indleader]
            # ~ print(u2_part.shape, u2t_part.shape)
            u2_measure_part = cp.vdot(u2_part,u2t_part)
            ########################### MPI ###########################
            # 此处向量会乱掉
            u2t_allpart = leader_comm.allgather(u2t_part)
            u2t = cp.concatenate(u2t_allpart) # H|u2>
            u2_measure = leader_comm.allreduce(u2_measure_part,op=MPI.SUM)
            ###########################################################
            a[ia]=(u2_measure.real)
            u3 = u2t - a[ia]*u2
            if ia>0: 
                u3 -= b[ia-1]*u1
            overlap = Q.conj()@u3
...
if __name__=='__main__':
    ...
    if rank==0:
        print('+==================================================================================+')
        print(f'+           Running in parallel on {size:4d} CPUs || Detected {num_gpus} GPU(s)           ')
    ...
    for Ktot in range(L):
        ...
        if algo == 0:
            en_lanczos[Ktot] = lanczos_mpi_cupy(indb_subset,basis,invbasis,num,numiteration)
        if algo == 6:
            en_lanczos[Ktot] = lanczos_reortho_mpi_cupysub(indb_subset,basis,invbasis,num,numiteration)
        elif algo == 1:
            en_lanczos[Ktot] = lanczos_reortho_mpi_cupy(indb_subset,basis,invbasis,num,numiteration,ortho_tol=1e-6)
        elif algo == 2:
